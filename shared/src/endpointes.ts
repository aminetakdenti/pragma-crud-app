...
export function withParams(endPoint: EndpointConfig, ...params: string[]): EndpointConfig {
  ...
  const placeholder = url.match(/:[^\/]*/g) || [];
  if (placeholder.length !== params.length) {
    throw `Too ${placeholder.length < params.length ? 'many' : 'few'} params for url: ${url}`;
  }

  for (let index = 0; index < params.length; index++) {
    url = url.replace(placeholder[index], params[index]);
  }
